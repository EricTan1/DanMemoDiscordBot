...
# add SA cap to 56 like holding SA (14*4)
...
async def calculate():
    ...
    adventurer_order = ["determination","","","","",""]
    ...
    for x in range(len(assists_order)):
        skill = [skilleffect for skilleffect in cache.get_assist_sa_gauge() 
                        if assists_order[x].lower() == skilleffect.title.lower()]
        if(len(skill) > 0):
    ...
    for x in range(len(adventurer_order)):
        skill = [skilleffect for skilleffect in cache.get_adventurer_sa_gauge() 
                        if adventurer_order[x].lower() == skilleffect.title.lower()]
        if(len(skill) > 0):
