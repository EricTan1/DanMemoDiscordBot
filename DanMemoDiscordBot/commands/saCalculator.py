...
async def run(client: WaitForClient, ctx: CommandContext, config_file: Optional[interactions.Attachment]):
    ...
    if(not config_file):
        await ctx.send("For this to work, you need to download the file, edit it, and reupload it into the channel with ais bot in it with the description /sa-calculator", files=interactions.File("sacalc.txt"))
    else:
        ...
        for line in contents_decode:
            ...
            #assist title format list
                ...
                if(len(assists_order) != 6):
                    errors += "Unable to read assist, make sure there are exactly 5 commas\n"
            ...
            # Turn orders for non sacs
                ...
                for position in range(4):
                    ...
                    if(stripped_line.startswith(line_start)):
                        ...
                        try:
                            positions[position] = verifyAndCast(stripped_line.split(","))
        ...
        # error handling
        if (errors == ""):
            await calculate(ctx,is_revis,adventurer_order,assists_order,positions,turns)
        else:
...
async def calculate(ctx: CommandContext, is_revis: bool, adventurer_turns: List[str], assist_turns: List[str], positions: List[List[int]], turns: int):
    curr_message = ""
    cache = Cache()
    ...
    while current_turn < turns:
        ...
        if(current_turn == 0):
            ...
            for index in [0,2,3,4]:
                calculate_assist_sa_gauge(assist_order, mlb_assists, sa_gauge_as, [0,2,3,4], index)
        # second sac
        elif(current_turn == 1):
            calculate_assist_sa_gauge(assist_order, mlb_assists, sa_gauge_as, list(range(1,5)), 1)
        # last person comes in
        elif(current_turn == 2):
            calculate_assist_sa_gauge(assist_order, mlb_assists, sa_gauge_as, list(range(2,6)), 5)
        ...
        for index in range(len(adventurer_order)):
            if(adventurer_order[index]):
                ...
                if(adv_skill.target.lower() == "self"):
                    if(not(turn_orders[index][current_turn] != 4 and adv_skill.skilltype.lower() == "special")):
        ...
        for x in range(2,5):
            if(turn_orders[x][current_turn] != 4):
                current_sa_gauge = current_sa_gauge + sa_gauge_adv[x] + sa_gauge_as[x] +1
                current_sa_gauge = round(current_sa_gauge,1)
            else:
        ...
        curr_message = curr_message + f"End of turn {current_turn+1} with SA gauge charge {round(current_sa_gauge/14,2)}\n\n"
...
def calculate_assist_sa_gauge(assists_order: List[List[SimpleNamespace]], mlb_assists: List[bool], charge_buffs: List[float], positions: List[int], self_pos: int):
