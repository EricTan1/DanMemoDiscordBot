...
async def run(client: WaitForClient, ctx: CommandContext, character: str):
    ...
    temp_embed.set_author(name="Akuno#8965",url=f"https://discordapp.com/users/{USER_ID}",icon_url=avatar_url)
    temp_embed.set_image(url=f"attachment://{character}.png")
    await ctx.send(embeds=temp_embed, files=interactions.File(f"./rbguides/{character}.png"))
