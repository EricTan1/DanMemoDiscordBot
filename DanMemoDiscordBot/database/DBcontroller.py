...
class DBcontroller:
    def __init__(self, config):
        ...
        self._mycursorprepared: MySQLCursorPrepared = self._connection.cursor(
            prepared=True
        )
    ...
    def getAdventurerName(self, adventurerid):
        print(adventurerid)
        sql = "SELECT a.title, c.name FROM danmemo.character as c, danmemo.adventurer as a WHERE a.adventurerid={} and c.characterid = a.characterid".replace(
    ...
    def getAssistName(self, assistid):
        sql = "SELECT a.title, c.name FROM danmemo.character as c, danmemo.assist as a WHERE a.assistid={} and c.characterid = a.characterid".replace(
            "danmemo", self.database
        ).format(
            assistid
        )
    ...
    def deleteById(self, entityname, column, value):
        ...
        sql = "DELETE FROM {}.{} WHERE {}=%s".format(
            self.database.lower(), entityname.lower(), column.lower()
        )
    ...
    def getDataColumnList(self, entityname, column):
        ...
        sql = "SELECT {} FROM {}.{} ".format(
            column, self.database.lower(), entityname.lower()
        )
    ...
    def characterSearch(self, search):
        ...
        for words in words_list:
            # adventurerid
            words = "%{}%".format(words)
            characterAdTitleSql = "SELECT adventurerid, a.title, c.name from danmemo.adventurer as a, danmemo.character as c where (c.name like %s or a.title like %s or a.alias like %s) and c.characterid = a.characterid".replace(
                "danmemo", self.database
            )
        ...
        for words in searchwords_list:
            ...
            if words.lower() in ele_set:
                ad_skill_effects_ret = [
                    skilleffect
                    for skilleffect in ad_skill_effects
                    if new_words == skilleffect.element.lower()
                ]
                as_skill_effects_ret = [
                    skilleffect
                    for skilleffect in as_skill_effects
            ...
            av_skill_effects_ret = [
                skilleffect
                for skilleffect in ad_dev_effects
                if new_words in skilleffect.development.lower()
            ]

            av_skill_effects_ret_2 = [
                skilleffect
                for skilleffect in ad_dev_skill_effects
    ...
    def assembleAdventurer(self, adventurerid):
        ...
        title_name = ""
        skill = []
        adventurer_base_sql = "SELECT title, c.name, limited, ascended,stars,t.name FROM danmemo.adventurer as a, danmemo.character as c, danmemo.type as t where c.characterid=a.characterid and t.typeid = a.typeid and a.adventurerid={}".replace(
            "danmemo", self.database
        ).format(
            adventurerid
        )
        skill_id_sql = "SELECT adventurerskillid FROM danmemo.adventurerskill where adventurerid = {}".replace(
            "danmemo", self.database
        ).format(
            adventurerid
        )
    ...
    def assembleAssist(self, assistid):
        ...
        instant_skill = []
        skill = []
        assist_base_sql = "SELECT title, c.name, limited,stars FROM danmemo.assist as a, danmemo.character as c where c.characterid=a.characterid and a.assistid={}".replace(
            "danmemo", self.database
        ).format(
            assistid
        )
    ...
    def assembleAssistSkill(self, skillid) -> Tuple[str, str, str]:
        ...
        skillname = ""
        skilltype = ""
        skill_sql = "SELECT skillname, skilltype FROM danmemo.assistskill where assistskillid={}".replace(
            "danmemo", self.database
        ).format(
            skillid
        )
        effects_sql = "SELECT t.name,m.value,a.name,e.duration, e.maxActivations, ele.name, ty.name\
            ...
            where assistskillid={}".format(
            *((self.database.lower(),) * 6), skillid
        )
    ...
    def assembleAdventurerDevelopment(
        ...
        effects_sql = "SELECT ta.name AS target, m.value AS modifier,  a.name AS attribute, adse.duration, ty.name AS type, e.name AS element, s.name AS speed\
            ...
            *((self.database.lower(),) * 7), adventurerDevelopmentid
        )
    ...
    def assembleAdventurerDevelopmentFromAdId(self, adventurerid):
        self._mycursor.execute(
            "SELECT ad.name, ad.adventurerdevelopmentid FROM danmemo.adventurerdevelopment as ad LEFT JOIN danmemo.adventurer as adv on adv.adventurerid = ad.adventurerid LEFT JOIN danmemo.character as c on adv.characterid= c.characterid WHERE adv.adventurerid = {}".replace(
                "danmemo", self.database
            ).format(
                adventurerid
            )
    ...
    def assembleAdventurerStats(self, adventurerid):
        ret_dict = dict()
        self._mycursor.execute(
            "SELECT a.name,stats.value FROM danmemo.adventurerstats as stats LEFT JOIN danmemo.attribute as a ON stats.attributeid = a.attributeid where stats.adventurerid = {};".replace(
                "danmemo", self.database
            ).format(
                adventurerid
            )
    ...
    def getAdSkillIdFromEffect(self, adventurerskilleffectsid):
        self._mycursor.execute(
            "SELECT AdventurerSkillid FROM danmemo.adventurerskilleffects WHERE AdventurerSkillEffectsid={}".replace(
                "danmemo", self.database
            ).format(
    ...
    def getAsSkillIdFromEffect(self, assistskilleffectsid):
        self._mycursor.execute(
            "SELECT assistSkillid FROM danmemo.assistskilleffects WHERE assistSkillEffectsid={}".replace(
                "danmemo", self.database
            ).format(
    ...
    def getAdventurerIdFromSkill(self, skillid: int) -> int:
        adventurer_base_sql = "SELECT adventurerid from danmemo.adventurerskill where adventurerskillid={}".replace(
            "danmemo", self.database
        ).format(
            skillid
        )
    ...
    def getAssistIdFromSkill(self, skillid: int) -> int:
        assist_base_sql = (
            "SELECT assistid from danmemo.assistskill where assistskillid={}".replace(
                "danmemo", self.database
            ).format(skillid)
        )
        self._mycursor.execute(assist_base_sql)
    ...
    def assembleAssistStats(self, assistid):
        ret_dict = dict()
        self._mycursor.execute(
            "SELECT a.name,stats.value FROM danmemo.assiststats as stats LEFT JOIN danmemo.attribute as a ON stats.attributeid = a.attributeid where stats.assistid = {};".replace(
                "danmemo", self.database
            ).format(
                assistid
            )
    ...
    def assembleAdventurerCharacterData(self, adventurerid):
        ret = ""
        adventurer_base_sql = "SELECT title, c.name, limited, ascended,stars FROM danmemo.adventurer as a, danmemo.character as c where c.characterid=a.characterid and a.adventurerid={}".replace(
            "danmemo", self.database
        ).format(
            adventurerid
        )
    ...
    def get_user(self, discord_id, discord_unique_id):
        ...
        for row in self._mycursor:
            ...
            user = database.entities.User.User(
                user_id,
                discordid,
                crepes,
                last_bento_date,
    ...
    def get_top_users(self, category):
        ...
        sql = "SELECT user_id, discord_id, crepes, last_bento_date, units, units_summary, gacha_mode, discord_unique_id, units_distinct_number, units_score FROM {}.user user WHERE user.{} > 0 ORDER BY user.{} DESC".format(
    ...
    def log_command(self, log_command):
        sql = "INSERT INTO {}.logs_command (discord_id, date, command, query, parameters) VALUES (%s,%s,%s,%s,%s)".format(
            self.database
        )
        parameters = (
            log_command.discord_id,
            log_command.date,
            log_command.command,
            log_command.query,
    ...
    def get_all_adventurers(self):
        sql = "SELECT a.adventurerid, a.characterid, a.typeid, a.alias, a.title, a.stars, a.limited, a.ascended, \
              ...
              {}.character AS c,\
              {}.type AS t\
              WHERE c.characterid = a.characterid AND t.typeid = a.typeid".format(
            self.database.lower(), self.database.lower(), self.database.lower()
        )
    ...
    def get_all_assists(self):
        sql = "SELECT a.assistid, a.characterid, a.alias, a.title, a.stars, a.limited, c.name, c.iscollab\
            ...
            self.database.lower(), self.database.lower()
        )
    ...
    def get_all_adventurers_developments_skills(self):
        ...
        FROM {}.adventurerdevelopment as addev\
        INNER JOIN {}.adventurer as ad on ad.adventurerid = addev.adventurerid\
        INNER JOIN {}.character as c on c.characterid = ad.characterid".format(
            *((self.database.lower(),) * 3)
        )
        ...
        for row in self._mycursor:
            ...
            row_as_dict = format_row_as_sns(
                adventurerdevelopmentid=adventurerdevelopmentid,
                unit_type=unit_type,
    ...
    def get_all_adventurers_developments_skills_effects(self):
        ...
        for row in self._mycursor:
            ...
            row_as_dict = format_row_as_sns(
                adventurerdevelopmentskilleffectsid=adventurerdevelopmentskilleffectsid,
                adventurerdevelopmentid=adventurerdevelopmentid,
                unit_type=unit_type,
                duration=duration,
                element=element,
    ...
    def get_all_adventurers_skills(self):
        ...
        for row in self._mycursor:
            ...
            row_as_dict = format_row_as_sns(
                adventurerskillid=adventurerskillid,
                adventurerid=adventurerid,
                skillname=skillname,
                skilltype=skilltype,
            )
    ...
    def get_all_adventurers_skills_effects(self):
        ...
        for row in self._mycursor:
            ...
            row_as_dict = format_row_as_sns(
                adventurerskilleffectsid=adventurerskilleffectsid,
                adventurerskillid=adventurerskillid,
                unit_type=unit_type,
                duration=duration,
                element=element,
    ...
    def get_all_assists_skills_effects(self):
        ...
        INNER JOIN {}.assistskill as ass on ass.assistskillid = ase.assistskillid\
        INNER JOIN {}.assist as assist on assist.assistid = ass.assistid\
        INNER JOIN {}.character as c on c.characterid = assist.characterid".format(
            *((self.database.lower(),) * 9)
        )
        ...
        for row in self._mycursor:
            ...
            row_as_dict = format_row_as_sns(
                assistskilleffectsid=assistskilleffectsid,
    ...
    def get_all_assist_sa_gauge_charge(self):
        sql = "SELECT askill.skillname,assist.title,chara.name,ta.name, mo.value FROM\
                ...
                where attributeid = 374;".format(
            *((self.database.lower(),) * 6)
        )
    ...
    def get_all_adventurer_stats(self):
        sql = "SELECT adventurerstatsid, adventurerid, advstats.attributeid, attri.name, value FROM {}.adventurerstats as advstats\
                LEFT JOIN {}.attribute as attri on attri.attributeid=advstats.attributeid;".format(
            *((self.database.lower(),) * 2)
        )
        ...
        for row in self._mycursor:
            ...
            row_ad_dict = format_row_as_sns(
                adventurerstatsid=adventurerstatsid,
                adventurerid=adventurerid,
    ...
    def get_all_assist_stats(self):
        sql = "SELECT assiststatsid, assistid, asstats.attributeid, attri.name, value FROM {}.assiststats as asstats\
                LEFT JOIN {}.attribute as attri on attri.attributeid=asstats.attributeid".format(
            *((self.database.lower(),) * 2)
        )
